...
class Post:
    ...
    def get_post(cls, data):
        ...
        results = connectToMySQL("thought").query_db(query, data)
        if len(results) < 1:
            return False
    ...
    def delete(cls, data):
        ...
        return connectToMySQL("thought").query_db(query, data)
    ...
    def get_all_posts(cls):
        ...
        results = connectToMySQL("thought").query_db(query)
        all_posts = []
        for result in results:
            new_post = True
            like_user_data = {
    ...
    def get_user_posts(cls, data):
        ...
        posts = connectToMySQL("thought").query_db(query, data)
        results = []
        for post in posts:
    ...
    def validate_post(data):
        ...
        if len(data['content']) < 4:
            flash("Thought is required, should be at least 5 characters", "error")
            is_Valid = False
        return is_Valid